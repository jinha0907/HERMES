...
const MainPage = () => {
  return (
    <div className="w-[1024px] h-[600px] mx-auto flex flex-col bg-gray-900 text-white p-6">
      {/* 상단 알림 목록 (4개 고정, 스크롤 없음) */}
      <div className="flex flex-col flex-none space-y-4 mb-6">
        {dummyAlarms.slice(0, 4).map((alarm) => (
          <Link
            key={alarm.id}
            to={`/alarms/${alarm.id}`}
            className="block bg-gray-700 rounded-2xl p-5 text-center text-2xl font-semibold shadow-md"
          >
            {alarm.title}
          </Link>
        ))}
      ...
      <div className="grid grid-cols-2 gap-4 flex-none">
        ...
        <div className="flex flex-col justify-between bg-gray-700 rounded-2xl p-6 shadow-lg text-center">
          <Link
            to="/settings"
            className="block w-full py-4 mb-3 bg-gray-400 rounded-2xl text-2xl font-bold"
          >
            환경 설정
          </Link>
          <Link
            to="/alarms"
            className="block w-full py-4 mt-3 bg-blue-500 rounded-2xl text-2xl font-bold"
          >
            알림 리스트
          </Link>
        </div>
